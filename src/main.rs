...
fn handle_post_request(request: &str) -> (String, String) {
    match (get_user_request_body(&request), Client::connect(DB_URL, NoTls)) {
        (Ok(user), Ok(mut client)) => {
            client
                .execute(
                    "INSERT INTO users (sabor, nombre, telefono) VALUES ($1, $2, $3)",
                    &[&user.sabor, &user.nombre, &user.telefono]
                )
                .unwrap();
...
fn handle_put_request(request: &str) -> (String, String) {
        ...
        )
    {
        (Ok(id), Ok(user), Ok(mut client)) => {
            client
                .execute(
                    "UPDATE users SET sabor = $1, nombre = $2, telefono = $3 WHERE id = $4",
                    &[&user.sabor, &user.nombre, &user.telefono, &id,]
                )
                .unwrap();
